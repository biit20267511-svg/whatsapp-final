...
import { LayoutDashboard, ShoppingBag, Users, UtensilsCrossed, MessageCircle, MessagesSquare, CalendarClock, Settings, Sparkles, LogOut, Pizza, CreditCard, Sheet, KeyRound } from "lucide-react";
import { apiGet, type WhatsAppConfig } from "@/lib/api";
import { useAuth } from "@/context/AuthContext";
import { ConnectionBadge } from "@/components/StatusBadge";
import ThemeToggle from "@/components/ThemeToggle";

const navigation = [["/dashboard", "Dashboard", LayoutDashboard], ["/orders", "Orders", ShoppingBag], ["/reservations", "Reservations", CalendarClock], ["/conversations", "Live Chat", MessagesSquare], ["/customers", "Customers", Users], ["/menu", "Menu", UtensilsCrossed], ["/whatsapp", "WhatsApp", MessageCircle], ["/google-sheets", "Google Sheets", Sheet], ["/billing", "Billing", CreditCard], ["/settings", "Restaurant Settings", Settings], ["/ai-settings", "AI Settings", Sparkles], ["/change-password", "Security", KeyRound]] as const;
export default function DashboardLayout({ children }: { children: ReactNode }) {
  ...
  return <div data-testid="dashboard-shell" className="noise-bg flex h-screen overflow-hidden bg-background text-foreground">
    <aside className="hidden h-screen w-64 shrink-0 flex-col overflow-y-auto border-r border-sidebar-border bg-sidebar md:flex">
    ...
    <main key={location.pathname} className="page-enter h-screen min-w-0 flex-1 overflow-y-auto p-5 md:p-8 lg:p-10"><div className="mb-6 flex items-center justify-between md:hidden"><p className="font-heading text-lg font-bold">{session?.restaurant?.name}</p><ConnectionBadge status={waQuery.data?.status} /></div>{children}</main>
  </div>;
}
