...
export default function AdminLayout({children}:{children:ReactNode}) {
  ...
  return <div data-testid="admin-shell" className="noise-bg flex h-screen overflow-hidden bg-background text-foreground">
    <aside className="hidden h-screen w-64 shrink-0 flex-col overflow-y-auto border-r border-white/10 bg-[#0D1111] text-white md:flex">
    ...
    <main key={location.pathname} className="page-enter h-screen min-w-0 flex-1 overflow-y-auto p-5 md:p-9">{children}</main>
  </div>;
}
