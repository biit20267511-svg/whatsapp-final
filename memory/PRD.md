...
## Iteration 8 — Auth hardening, Delivery Zones, Pakistani Menu UX, Pickup flow (June 2026)
- **Auth**: password policy (8+ chars, letters+digits), username policy, 5-strike / 15-min lockout (`login_attempts`), `token_version` on every user → any credential change (admin panel, reset, self change) revokes all old JWTs (401 → frontend auto-logout to `/login?reason=session`). `POST /auth/change-password` + `/change-password` page (forced when `must_change_password`; owner nav "Security"). Admin profile update returns fresh token; admin restaurant credentials card shows "Share with owner" copy box + must-change badge. Seed no longer overwrites owner password on restart; super admin email from `SUPER_ADMIN_EMAIL` (migrates legacy admin). Tests read `SUPER_ADMIN_EMAIL` env.
- **Delivery zones** (Restaurant Settings → Delivery): `delivery_enabled`, `delivery_mode` fixed|zones, `restrict_to_zones`, `delivery_zones[{id,name,aliases,fee,min_order,eta_min,active}]`. AI tool `check_delivery_area`; address auto-zone detect; outside area → refuse + pickup offer (or default fee if not restricted); zone fee/min-order/ETA applied in totals; order stores `delivery_zone`.
- **Menu**: category templates chips + `POST /menu/categories/bulk`; Quick Add (`POST /menu/items/bulk`, formats `Name 1200`, `Name | Half 700 / Full 1300`, `Name - 650 - desc`); item `variants[]` & `addons[]` (AI asks size, adds add-on prices; bulk price adjust scales variants). AI settings page: switches, own API key, test connection.
- **Orders**: `Picked Up` status; pickup flow New→Confirmed→Preparing→Ready→Picked Up (backend rejects Out for Delivery/Delivered for pickup and Picked Up for delivery); board `Completed` column; Ready-for-pickup WhatsApp message with restaurant name/address/order # ; Picked Up thank-you. Sidebar fixed (`h-screen overflow-hidden`, main scrolls).
- Verified: pytest 57/57 (incl. new `test_iter8_auth_zones_menu.py`), tsc clean, testing agent iteration_8 frontend 100%. Known: Emergent LLM proxy intermittently returns 403 `FREE_USER_EXTERNAL_ACCESS_DENIED` under burst load → AI fallback message (platform-side, not code).
- Backlog: menu item aliases (Roman Urdu) for AI, deals/combo builder, WhatsApp menu preview, refresh tokens/short-lived access tokens, mobile sidebar drawer.
